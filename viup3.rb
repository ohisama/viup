...
module VU3
  def self.export_entities(of, id, entities, o = nil, emat = nil)
    ...
    faces.each do |f|
      mesh = f.mesh 5
      (1..mesh.count_points).each { |p|
        verts << mesh.point_at(p)
      }
    end
    verts.flatten!
    verts.uniq!
    #Sketchup.set_status_text("Find vertes " + verts.length.to_s)
    j = 0
    verts.each { |pt|
      pt = pt.transform(o) if o
      vf = format("%8.6f %8.6f %8.6f", pt.x.to_f / 100, pt.z.to_f / 100, -pt.y.to_f / 100)
      of.print "v #{vf}\n"
      j += 1
      #Sketchup.set_status_text("out vertes " + j.to_s)
    }
    faces.each do |f|
      mata = nil
      matb = nil
    ...
    faces.each do |f|
      ...
      if texFile
        polys.each { |p|
          of.print "f "
          pt = mesh.point_at(p[0])
          i = verts.index(pt) + @verts
          cnt += 1
          of.print "#{i }/#{cnt + @uvcnt} "
          pt = mesh.point_at(p[1])
          i = verts.index(pt) + @verts
          cnt += 1
          of.print "#{i }/#{cnt + @uvcnt} "
  ...
  def self.obj_export
    ...
    UI.messagebox @spath
    UI.openURL("file://" + @spath + "/ohisama/run3.bat")
  end
end
if (not file_loaded?("viup3.rb"))
  begin
    Ohi_menu.add_item("viup3") {VU3::obj_export}
  rescue
    Ohi_menu = UI.menu("Plugins").add_submenu("Ohisama")
    Ohi_menu.add_item("viup3") {VU3::obj_export}
  end
end
file_loaded("viup3.rb")
